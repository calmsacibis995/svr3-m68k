...
strinit()
{
	...
	if (!(base = (unsigned char *)sptalloc(size, PG_VALID, 0, NOSLEEP))) {
		goto alcfail;
        }
...
retry:
	 ...
	 */
	while ((sep = (struct strevent *)sptalloc(1, PG_VALID, 0,
		(slpflag == SE_NOSLP) ? NOSLEEP : 0)) == NULL) {
		cmn_err(CE_WARN, "KERNEL:sealloc: Not enough memory for page allocation\n");
		if (slpflag == SE_NOSLP) {
			sepgcnt--;
			splx(s);
			return(NULL);
