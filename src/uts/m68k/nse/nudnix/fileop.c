/*
 * $Header: /rl/sys/nudnix/RCS/fileop.c,v 1.3 86/09/08 10:29:14 dap Exp $
 * $Locker:  $
 */
...
register flag, offset;
{
	...
	switch (u.u_syscall) {
	case	DULOCKING: {
		register struct a { int fdes; int cmd; int size; } *uap;
		...
		req->rq_cmd     = uap->cmd;	/* rq_arg[0]	*/
		req->rq_fflag   = flag;		/* rq_arg[3]	*/
		req->rq_foffset = offset;	/* rq_arg[2]	*/
		req->rq.size    = uap->size;
		break;
	}
