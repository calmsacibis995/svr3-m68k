...
char	**list;
{
	...
	for (i=0; i < MAXNAMES; i++) {
		...
		if ((int) (alist[i] = (char *) fuword(list++)) == -1) {
...
struct idtab	*map;
{
	...
	/* DO NOT print string uname as this is a user pointer
	   value is used for test so print that */
	DUPRINT4(DB_RFSYS,"setidmap: uname=%x, flag=%d, map=%x\n",
		name,flag,map);
	...
	if (map) {
		if ((ref = h_alloc(sizeof(struct idtab) *
...
struct gdp	*gp;
{
	DUPRINT2(DB_RFSYS,"remidmap: gp=%x\n",gp);

	if (gp->idmap[UID_DEV]) {
		freeidmap(gp->idmap[UID_DEV]);
		gp->idmap[UID_DEV] = NULL;
	}
	if (gp->idmap[GID_DEV]) {
		freeidmap(gp->idmap[GID_DEV]);
...
struct stat	*sptr;
{
	sptr->st_uid = rglid(UID_DEV,gdpp,sptr->st_uid);
	sptr->st_gid = rglid(GID_DEV,gdpp,sptr->st_gid);
	return;
}
ushort
