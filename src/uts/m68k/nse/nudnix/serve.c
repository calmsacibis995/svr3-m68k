/*
 * $Header: /rl/sys/nudnix/RCS/serve.c,v 1.3 86/09/08 10:28:57 dap Exp $
 * $Locker:  $
 */
...
/* position of param.h changed so ip->fstypp defined */
...
serve ()
{
	...
	psmsg:
		...
		case DULOCKING:
			DUPRINT5(DB_FSS
				 , "locking: cmd=%d flag=%d off=%d size=%d\n"
				 , msg_in->rq_cmd
				 , msg_in->rq_fflag
				 , msg_in->rq_foffset
				 , msg_in->rq.size);
			...
			if (setjmp(u.u_qsav))
				...
				FS_LOCKING(queue->rd_inode
					   , msg_in->rq_cmd	/* rq_arg[0] */
					   , msg_in->rq_fflag	/* rq_arg[3] */
					   , msg_in->rq_foffset	/* rq_arg[2] */
					   , msg_in->rq.size);

			DUPRINT2(DB_FSS, "locking: errno=%d\n", u.u_error);
