...
daddr_t bn;
{
	...
	for (i = FsNINDIR(mp->m_bsize) - 1; i >= 0; i--) {
#ifdef	FSMK
		if(bp != NULL && (i & FsNMASK(1024) == FsNMASK(1024))){
			brelse(bp);
			bp = NULL;
		}
#endif
		if (bp == NULL) {
#ifdef	FSMK
bp = bread(dev, bn*(mp->m_bsize/1024)+(i>>FsNSHIFT(1024)), FsBSIZE(mp->m_bsize));
#else
...
struct mount s5_8192mnt;
struct mount s5_4096mnt;
...
s5init()
{
	...
	s5ip->s5i_next = NULL;
	s5ip->s5i_flags = S5IFREE;
	s5_8192mnt.m_bsize = 8192;
	s5_4096mnt.m_bsize = 4096;
