...
#ident	"@(#)kern-port:fs/s5/s5alloc.c	10.6"
#include "sys/types.h"
...
#ifdef	FSMK
		if(mp->m_bsize > 1024){
			bp = bread(dev, bno*(mp->m_bsize/1024), FsBSIZE(mp->m_bsize));
		}
		else
#endif
...
#ifdef	FSMK
	bp = getblk(dev, bno * (mp->m_bsize/1024), FsBSIZE(mp->m_bsize));
#else
...
#ifdef	FSMK
	else if (bp = (struct buf *)incore(dev, bno*(mp->m_bsize/1024), FsBSIZE(mp->m_bsize)))
#else
	else if (bp = (struct buf *)incore(dev, bno, FsBSIZE(mp->m_bsize)))
#endif
...
loop:
	...
	if (fp->s_tinode > 0) {
		...
		for (adr = FsITOD(mp->m_bsize, ino); adr < fp->s_isize*(mp->m_bsize/1024); adr++) {
