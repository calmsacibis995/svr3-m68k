...
#ifdef COALES
				if(coal_enb){
					u.u_rablock = 0x80000000 | (tune.t_maxrdahead);   
#if DEBUG
if(bmap_debug)
					printf("s5bmap: readahead cnt %x blkno %x\n", u.u_rablock, bn);
#endif
				}
				else
#endif
		...
		if (bn < NADDR-3) {
			...
			if (nb == 0) {
				if (readflg || (bp = s5alloc(mp))==NULL)
					return((daddr_t)-1);
				if(mp->m_bsize >1024)
					nb = FsPTOL(bsize, bp->b_blkno/(mp->m_bsize/1024));
				else
					nb = FsPTOL(bsize, bp->b_blkno);
#ifdef	FSMK
				clrblk= nb * mp->m_bsize/1024;
#endif
...
#ifdef	FSMK
				clrblk++;
				for(mblks=(mp->m_bsize/1024)-1; mblks>0; mblks--){
					bp=getblk(dev, clrblk,1024);
					clrbuf(bp);
					if ((s5ip->s5i_mode&IFMT) == IFDIR){
						bwrite(bp);
					}
			...
			if ((i < NADDR-4) && raflag)
			   ...
			   if(coal_enb) {
				racount = 0;
				for(n=1; n <= NADDR -3 -i; n++) {
					if(s5ip->s5i_addr[i+n] == nb + n)
						racount++; /* read ahead up to 9 blocks */
					else
						break;
				}
#ifdef	FSMK
				racount *= mp->m_bsize/1024;
				racount += mp->m_bsize/1024-1 - ((u.u_offset >>10) & (mp->m_bsize/1024-1));
#endif
				if(racount)
				{
#ifdef	FSMK
					if(racount > tune.t_maxrdahead)
						racount = tune.t_maxrdahead;
#endif
					u.u_rablock = 0x80000000 | racount;
#if DEBUG
if(bmap_debug)
					printf("bmap: read ahead drct cnt %x blk %x\n", u.u_rablock, nb);
#endif
				}
				else
#ifdef	FSMK
					u.u_rablock = s5ip->s5i_addr[i+1]*(mp->m_bsize/1024);
#else
			...
			if(mp->m_bsize > 1024){
				nb *= mp->m_bsize/1024;
				nb += (u.u_offset >> 10) & (mp->m_bsize/1024-1);
				return(nb);
			}
			else
#endif
...
#if DEBUG
if(bmap_debug)
						printf("bmap: read ahead indir cnt %x blk %x\n", u.u_rablock, nb);
#endif
					}
					else
#ifdef	FSMK
						u.u_rablock = bap[i+1] * (mp->m_bsize/1024);
#else
						u.u_rablock = bap[i+1];
#endif
			...
			if (nb == 0) {
				...
				if(mp->m_bsize >1024)
					nb = FsPTOL(bsize, nbp->b_blkno/(mp->m_bsize/1024));
				else
