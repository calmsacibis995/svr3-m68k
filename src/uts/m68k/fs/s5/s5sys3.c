...
extern struct mount s5_8192mnt;
extern struct mount s5_4096mnt;
...
s5rmount(iflg)
{
	...
	if (pipedev == rootdev)
		pipedev = rootdev;
	if (iflg) {
		if(time < fp->s_time)  /* if rtc lost time setting */
			clkset(fp->s_time); /* user super block time */
		/* 
...
int  ufstyp;
{
	...
	if (fp->s_type == Fs2b) {
		...
		sp->f_bsize = 512;
#ifdef	FSMK
	} else if (fp->s_type == Fs16b) {
		sp->f_bsize = 8192;
	} else if (fp->s_type == Fs8b) {
		sp->f_bsize = 4096;
	} else if (fp->s_type == Fs4b) {
		sp->f_bsize = 2048;
#endif
	} else {
		u.u_error = EINVAL;
