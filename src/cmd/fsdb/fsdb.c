...
printf("trying to make node %s %d %d %ld\n",devnam,maj,min,blk);
	...
	/* check to see if this is a 1kb, 8kb or 512b file system */
...
#define S	sblock.fs
	...
	if ((S.s_type != Fs1b) && (S.s_type != Fs2b) && (S.s_type != Fs16b)) {
	...
	if (S.s_type == Fs16b) {
		close(fd);
		if (execvp("/etc/fsdb16b", argv) < 0) {
			printf("%s: cannot exec fsdb16b\n", argv[0]);
			exit(1);
		}
	}
	if (S.s_type == Fs2b) {
		close(fd);
		if (execvp("/etc/fsdb", argv) < 0) {
			printf("%s: cannot exec fsdb\n", argv[0]);
			exit(1);
		}
	}
		printf("%s(%s): 512 byte Block File System\n", argv[1], S.s_fname);
#endif
#if (FsTYPE==2 || FsTYPE==3)
	if (S.s_type == Fs1b) {
		close(fd);
	...
	printf("%s(%s): 8K byte Block File System\n", argv[1], S.s_fname);
#endif
#endif
...
#if pdp11 || m68k /* We don't force 4byte alignment of daddr_t's (longs) */
	addr = SUPERBOFF + 2;
#else  /* 3b2 does...[should change to '+ (int)(&((filsys *)0->s_fsize))] */
	addr = SUPERBOFF + 4;
#endif
