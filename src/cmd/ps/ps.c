...
int	nut = 0;		/* counter for uid_tbl */
					...
					/* swaplo(w): from /sysV68, 1st block # on */
struct nlist nl[] = {			/* 	      device to be used for swap */
#if u3b || u3b2 || u3b5 || m68k
...
int	ngrpid = 0;
int	curr_proc = 0;	/* number of procs read, used to
			calculate ADDR for m68k */

#if u3b || u3b2 || u3b5 || m68k				/* State flag symbols, see sys/proc.h */
...
char **argv;
{
	...
	for (i=0; i<v.v_proc; i++) {
		...
		curr_proc++;		/* number of procs read */
		if (mproc.p_stat == 0)	
			continue;
		puid = mproc.p_uid;
		ppid = mproc.p_pid;
...
int readata()
{
	...
	if (stat(psfile, &sbuf1) < 0
	    ...
	    || (stat("/sysV68", &sbuf2) < 0 || sbuf1.st_mtime <= sbuf2.st_mtime || sbuf1.st_mtime <= sbuf2.st_ctime)
...
#if pdp11 || u3b2 || u3b5 || m68k
	long addr;
#endif

	register char *cp;

#if pdp11 || u3b || u3b2 || m68k
	register char *cp1;
#endif
...
#ifdef m68k
	addr = ctob(pg_getpfn(mproc.p_ubpgtbl));
	file = swmem;
	l_lseek(file, addr, 0);
	if ((nbytes = read(file, (char *)&u, sizeof(u))) != sizeof(u))
		return(0);
...
#endif /*pdp11*/
#if m68k
		/* calculate phys. mem. addr. of page table map
		within its associated proc structure.  Use the
		local mproc structure to calculate sizes and 
		offset, applying those to known table address. */
		table_addr = nl[0].n_value + (((curr_proc - 1) *
		(sizeof(mproc))) + ((long)mproc.p_ubpgtbl - (long)&mproc));
		printf("%9x%7d", table_addr, mproc.p_size);
		if (mproc.p_wchan)
			printf("%9x", mproc.p_wchan);			/* WCHAN */
		else
			printf("         ");
