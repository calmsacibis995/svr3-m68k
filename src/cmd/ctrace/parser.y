...
/*
 * (mat) 10-24-86 --> X and Y casting in min and max are tempory for
 *		      a compiler bug, yuck.
 */
#define min(X, Y)	(((int) X < (int) Y) ? (enum symbol_type)X : (enum symbol_type)Y)
#define max(X, Y)	(((int) X > (int) Y) ? (enum symbol_type)X : (enum symbol_type)Y)
