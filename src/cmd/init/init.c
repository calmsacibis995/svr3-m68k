...
struct	termio	dflt_termio = {
	...
	CS8|CREAD|B9600,
	ISIG|ICANON|ECHO|ECHOK|ECHOE,
	0,0177,control(\\),CERASE,CKILL,control(D),0,0,0
};
#endif
...
#ifdef	MAXLOGINS
		if ((cmd.c_action == M_RESPAWN) && (++respcnt > MAXLOGINS))
			cmd.c_action = M_OFF;
#endif
...
#ifdef	MAXLOGINS
		if ((cmd.c_action == M_RESPAWN) && (++respcnt > MAXLOGINS)) {
		    if( error_time(MAXLTYPE) )
			console("Too many `respawn' entries in /etc/inittab\n");
		    cmd.c_action = M_OFF;
		}
#endif
...
/* code folded from here */
	if((cmd->c_levels & MASKSU) && !(act_masks[i] & (M_INITDEFAULT | M_PF | M_PWAIT)))
		cmd->c_action = 0;
...
struct PROC_TABLE *efork(action,process,modes)
int action;
register struct PROC_TABLE *process;
int modes;
{
