...
readregs()
{
	...
	{	int foffset, *faddr;
		char fsave;
		/* get first byte of save area */
		foffset = (int)&(((struct user *)0)->u_fpu.fsave[0]);
		fsave = (0xff & (ptrace(RUREGS,pid,foffset,0)) >> 24);
		/* if first byte of save area is non-zero, get fp regs */
		if(((struct user *)uu)->u_fpu.fsave[0] = fsave )
		{
			faddr = &(((struct user *)uu)->u_fpu.regs.reg[0][0]);
			foffset = (int)faddr -(int)uu;
			/* get fp regs into local copy of u */
			for(i=0; i<96; i+=4)
				*faddr++ = ptrace(RUREGS, pid, foffset+i,0);
		}
	}
#endif
#endif
