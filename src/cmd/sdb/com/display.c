...
int prvar,i; {
	...
	if(prvar < 2){
		if( ((struct user *)uu)->u_fpu.fsave[0] == 0)
			printf("floating point co-processor uninitialized\n");
		else 
		{
			if((i>=0) && (i<8)) {
		...
		case 2:		/* DBL */
		 ...
		 if((i>=0) && (i<8)){
			...
			dtox(&fpreg, addr);
			if(pid){
				ptrace(WUREGS, pid, (int)addr-(int)uu, *(long *)(addr));
				ptrace(WUREGS, pid, (int)addr-(int)uu+4, *(long *)(addr+4));
				ptrace(WUREGS, pid, (int)addr-(int)uu+8, *(long *)(addr+8));
			}
		}
		else{
			addr = (unsigned int)&(fpbuf[0]);
...
char *x, *d;
{
	...
	asm("mov.l 8(%fp),%a1");	/* addr of extended into a1 */
