...
CFLAGS = -O -DSMALL ${S5CFLAGS} -DNO_MODEM_CTRL
CC=cc
IFLAG =
LDFLAGS= ${IFLAG} ${S5LDFLAGS}
...
install:	copyright mkdirs all shells cp

shells:
	#	For cross environment, copy default files
	if [ -n "$(CH)" ]; then cp Devices Dialers Systems Permissions Poll Dialcodes Devconfig Sysfiles $(ROOT)/usr/lib/uucp/; fi
	$(CH)/bin/sh SetUp
	echo 2 > $(MAXUUXQTS)
	echo 2 > $(MAXUUSCHEDS)
...
cp:	all 
	...
	$(INS) $(OLD) uugetty $(INSDIR)/uugetty 544 $(OWNER) $(GRP)
