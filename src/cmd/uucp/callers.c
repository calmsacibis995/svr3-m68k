...
register char *flds[], *dev[];
{
	...
	if (dcf == -1) {
		...
		/* This sequence is required to allow uugetty and
		   cu/uucp/ct to work.  Sequence is
		   1.	open no delay
		   2.	stty clocal
		   3.	reset no delay
		   4.	reopen - forces wakeup of WOPEN guys in driver
		   5.	close
		*/
		fcntl(dcf,3,open_modes);
		open_modes &= ~O_NDELAY;
		fcntl(dcf,4,open_modes);
		nullfd = open(dcname,2);
		close(nullfd);
		(void) alarm(0);
	}
