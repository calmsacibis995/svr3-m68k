...
#ifdef m68k
#define STEPSIZE	1	/* default step for freelist spacing */
#else
...
char	rflag;			/* recover from track redirection */
...
#define BADBMAX 100  /* max files which can contain bad blocks */
...
char	*argv[];
{
	...
	for(i = 1; --argc < 0, *argv[i] == '-'; i++) {
		switch(*(argv[i]+1)) {
			...
			case 'r':
				rflag++;
				if(*argv[++i] == '-' || --argc <= 0)
					errexit("%c Bad -r option\n",id);
				strcpy(badbfile,argv[i]);
				init_blist();
				break;
			case 't':
			case 'T':
	...
	if(!argc) {		/* use default checklist */
	   if (rflag) {
		errexit("%c The 'r' option requires a single device name\n",id);
	   }
	   else {
	...
	(superblk.s_magic == FsMAGIC && superblk.s_type == Fs16b)) {
#endif
#if FsTYPE==1
...
#endif
#if FsTYPE==5
	if((superblk.s_magic == FsMAGIC && superblk.s_type == Fs1b) ||
	(superblk.s_magic == FsMAGIC && superblk.s_type == Fs2b)) {
#endif
			close(dfile.rfdes);
...
#if FsTYPE==2
			if (superblk.s_type == Fs16b) {
			    if(execvp("/etc/fsck16b",argv) == -1)
			       errexit("%c %sCannot exec /etc/fsck16b\n",
					id,devname);
			}
			else {
...
#if FsTYPE==2
			if(superblk.s_magic != FsMAGIC ||
			(superblk.s_magic == FsMAGIC && (superblk.s_type == Fs1b || superblk.s_type == Fs16b))) {
#ifdef u3b5
...
#if FsTYPE==5
			if(superblk.s_magic != FsMAGIC ||
			(superblk.s_magic == FsMAGIC && (superblk.s_type == Fs1b || superblk.s_type == Fs2b))) {
				error("%c %s not an 8k file system\n",
					id,argv[ix]);
				ix++;
				continue;
			}
...
#endif
#ifdef m68k
	memsize = BUFMULT*sizeof(BUFAREA);
#else
...
register daddr_t blk;
{
	...
	if (rflag) {
		if ((cell = find_blist(blk, NOADD)) != NOTFOUND)
			fill_blist(cell, inum, s_depth, e_depth);
	}
	if(getbmap(blk)) {
		blkerr("DUP",blk);
		if(++dupblk >= MAXDUP) {
...
ginode()
{
	...
	if (rflag) {
	    while (GOODBLK == NO) {
		if ((cell = find_blist(iblk, NOADD)) != NOTFOUND) {
			if (pfunc == pass1) {
			  	printf("\nI-BLOCK CORRUPT - BLOCK= %d",iblk);
			  	printf("\n  INODES %d", inum);
			}
			badblk = iblk;
			inum++;
			while ((iblk = itod(inum)) == badblk)
				inum++;
			if (pfunc == pass1) 
				printf(" - %d UNRECOVERABLE\n\n", inum-1);
		}
		else
	   		GOODBLK = YES;
	    }
...
int flg;
{
	...
	while ((ar_index<BADBMAX) && (badblist[ar_index].blknum!=bnum)
	&& (badblist[ar_index].blknum != 0))
		ar_index++;
	if (ar_index >= BADBMAX)
		return(NOTFOUND);
	else {
		if (badblist[ar_index].blknum == 0) {
			if (flg == ADD)
				badblist[ar_index].blknum = bnum;
			else
				return(NOTFOUND);
		}
		return(ar_index);
...
fill_blist(cell, inumber, s_depth, e_depth)
register int cell;
register int inumber;
register long s_depth;
register long e_depth;
{
	if ((cell >= BADBMAX)||(cell < 0)) {
		printf("Bad cell number!\n");
		exit(0);
	}
	badblist[cell].inum = inumber;
	badblist[cell].s_depth = s_depth;
	badblist[cell].e_depth = e_depth;
}

init_blist()
{
	register int index = 0;
	register char blocknum[10];  /* probably no more than 10 digits */
	long bnum;
	register FILE *fp;

	for (;index < BADBMAX; index++) {
		badblist[index].blknum = 0;
		badblist[index].inum = 0;
		badblist[index].s_depth = 0;
		badblist[index].e_depth = 0;
	}
	/* read from the file, fill in the table */
	if((fp = fopen(badbfile,"r")) == NULL)
		errexit("%c Can't open bad block file: %s\n",
			id,badbfile);
	index = 0;
	while(getline(fp,blocknum,sizeof(blocknum)) != EOF) {
		if(((bnum = strtol(blocknum, (char **)NULL, 0)) != 0) && (bnum != 1))
			if(++index > BADBMAX)
				errexit("%c MAXIMUM %d BAD BLOCKS ALLOWABLE\n", id, BADBMAX);
			find_blist(bnum, ADD);
	}
	fclose(fp);
}
...
int inumber;
{
	register int ar_index = 0;
	register int count = 0;

	for (;ar_index < BADBMAX; ar_index++) {
		if (badblist[ar_index].inum == inumber)
			count++;
	}
	return(count);
} 
