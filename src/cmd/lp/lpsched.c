...
SCCSID("@(#)lpsched.c	2.1	")

static int cancflg=FALSE;	/* This flag can only be set to TRUE in one of
				 *  lpsched's child processes (see makebusy()
				 *  and killchild()).  There is never a need
				 *  to reset this flag since the process that
				 *  changes it exits.  All new children of 
				 *  lpsched will always inherit this flag with
				 *  a value of FALSE. 081285  Mat C.
				 */
...
/*
 *      Modified by LCC Sep 19, 1985 to return a 0 or 1
 *      in errno if an error.  This is because the first
 *      "error condition" is a legitimate state if the
 *      r-file has been deleted, but we haven't been
 *      notified yet.  The other conditions are more
 *      serious and the user should be notified that
 *      his request has been deleted.
 */
...
cinit()
{
	...
	if((cd = fopen(CLASS, "r")) == NULL)
		fatal("can't open CLASS directory", 1);

	while(fread((char *) &dirbuf, sizeof dirbuf, 1, cd) == 1) {
...
cleanfiles()
{
	...
	FORALLD(d) {
		...
		while(fread((char *)&entry, sizeof(struct direct), 1, f) == 1) {
			if(entry.d_ino != 0 && entry.d_name[0] != '.') {
				strncpy(file, entry.d_name, DIRSIZ);
				*(file + DIRSIZ) = '\0';
				switch(*file) {
				case 'd': /* data file */
				case 'r': /* request file */
 ...
 */
/* Modified by LCC Sep 19, 1985
 *      The parent lpsched has SIGTERM set to come to
 *      cleanup.  There is a small window after the
 *      fork wherein the child could get sent here
 *      and delete FIFO thereby stopping all further
 *      print spool activity.  Since the parent of
 *      parent lpsched should be 1, if the parent's
 *      ID is not 1, we just exit.
 */
...
opendev()
{
	...
	if(eaccess(dev, ACC_W) != 0)
		...
		if(eaccess(dev, ACC_R) == 0)	/* open for r+w, if possible */
			mode[1] = '+';
		/*
		 * Allow closing of the line to complete with no time
		 * constraints.  Place the alarm of 
		 * OPENTIME around reopening of line.  The alarm use to be
		 * around the close and open which produced 
		 * erroneous errors and disabling of the printer on a 
		 * validly slow close.  (Mat C. 072985)
		 */
		signal(SIGALRM, sig14);
		f = NULL;
		/* Close line outside of timer loop */
		fclose(stdout);
		alarm(OPENTIME);
...
pinit()
{
	...
	if((md = fopen(MEMBER, "r")) == NULL)
		fatal("can't open MEMBER directory", 1);

	while(fread((char *) &dirbuf, sizeof dirbuf, 1, md) == 1) {
		if(dirbuf.d_ino != 0 && dirbuf.d_name[0] != '.') {
			d = newdest(dirbuf.d_name);
 ...
 *
 *	Modification	081285		Mat C.
 *
 *	A problem arose where the interface file contained a
 *	"trap statement 15" where statement contained a 
 *	"sleep 30".  Since SIGTERM was caught by the
 *	interface shell when a printing job was canceled, the
 *	status we get here does NOT reflect that the job was canceled.
 *	If another job was queued up while the trap statements were being 
 *	executed, in this case the sleep 30, postprint() would perform 
 *	the wrong kind of cleanup. This in turn caused FIFO and SCHEDLOCK 
 *	to be removed by one of lpsched's children (the way this happens is
 *	to involved to go into here).  The final state was that the
 *	scheduler was hung.
 *
 *	To avoid this problem, an assumption that the scheduler
 *	made was removed:
 *
 *		- the scheduler knew a job was canceled if and only if its 
 *		  return status from the interface file indicated that it
 *		  was killed by SIGTERM.
 *
 *	Now, as part of the signal handling routine for the child of
 *	lpsched who is waiting for the interface shell to complete,
 *	I have included the setting of a flag (see killchild()).
 *	When lpsched's child goes into its cleanup routine it
 *	uses this flag as an indication that the current job was
 *	canceled, thereby not relying on the status returned by
 *	the interface shell for this information.
 */
...
int status;
{
	...
	if(term == SIGTERM || excode == (EX_SYS | EX_TERM) || cancflg) {
...
preprint()
{
	...
	/*
	 * Modified by LCC Sep 19, 1985 to check errno before
	 * sending the error message via mail.  This was to
	 * eliminate the error message in case the request
	 * was cancelled.  If one of the other errors occured
	 * in buildcmd(), the error message is sent.
	 */
		if(errno) {
			sprintf(errmsg, "error in printer request %s-%d",
				dst, seqno);
			wrtmail(logname, errmsg);
		}
		resetstatus(1, 0);
...
killchild()
{
	signal(SIGTERM, SIG_IGN);
	kill(-pid, SIGTERM);
	cancflg = TRUE;		/* job currently printing is being canceled */
}
	...
	if((ppid = p->d_pid) != 0) {
		...
		while((ret=wait(&status)) != ppid) {
			if(ret == -1 && errno == ECHILD) {
				fprintf(stderr,"unexpected wait return\n");
				fflush(stderr);
				return;
			}
