...
char *argv[];
{
	...
	execargv = (char *)argv;  /* in case we need to exec after we've left main() */
	while (--argc) {
		argv++;
		if (**argv=='-')
		switch ((*argv)[1]) {
...
char *file;
{
	...
	if(sblock.s_magic == FsMAGIC && sblock.s_type == Fs2b) {
		if(execvp("/etc/ncheck",execargv) == -1)
			printf("Cannot exec /etc/ncheck");
	}
	if(sblock.s_magic == FsMAGIC && sblock.s_type == Fs16b) {
		if(execvp("/etc/ncheck16b",execargv) == -1)
			printf("Cannot exec /etc/ncheck16b");
	}
#endif
#if FsTYPE==2
	if(sblock.s_magic != FsMAGIC ||
	(sblock.s_magic == FsMAGIC && sblock.s_type == Fs1b)) {
		if(execvp("/etc/ncheck1b",execargv) == -1)
			printf("Cannot exec /etc/ncheck1b");
	}
	if(sblock.s_magic == FsMAGIC && sblock.s_type == Fs16b) {
		if(execvp("/etc/ncheck16b",execargv) == -1)
			printf("Cannot exec /etc/ncheck16b");
