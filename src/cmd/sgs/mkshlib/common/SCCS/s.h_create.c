h58800
s 00038/00001/00426
d D 1.2 86/08/18 08:48:56 fnf 2 1
c Checkpoint changes at first working shared library version
e
s 00427/00000/00000
d D 1.1 86/07/31 14:39:14 fnf 1 0
...
mklibdef()
{
	...
	long	saddr;		/* segment address of current section */
	int	i;
...
I 2
	DBUG_4 ("trgfil", "file '%s' open on trgfil=%#x", trgname, trgfil);
E 2
	/* open deffil */
...
I 2
	DBUG_4 ("deffil", "file '%s' open on deffil=%#x", defname, deffil);
E 2
	...
	for (i=0; i<fhead.f_nscns; i++) {
		...
		(void)sprintf(shead.s_name,".fk%x",saddr);
...
I 2
	DBUG_2 ("lib1", "write out the .lib section header and contents");
E 2
	if (!libfound)
	...
	/* write the section header to deffil */
I 2
	DBUG_2 ("lib2", "write out section header to deffil");
E 2
...
I 2
	DBUG_2 ("lib3", "copy .lib section contents of trglib to deffil");
E 2
...
I 2
	DBUG_2 ("lib4", "modify file header and write to deffil");
E 2
...
I 2
	DBUG_2 ("lib5", "create symbol and string table for deffil");
E 2
...
I 2
	DBUG_2 ("lib6", "close trgfil and deffil");
E 2
	(void)ldclose(trgfil);
	(void)fclose(deffil);
I 2
...
I 2
	DBUG_ENTER ("creatobjs");
	DBUG_2 ("cof1", "create modified object files");
E 2
	/* set up library definition symbol entry */
I 2
	DBUG_2 ("cof2", "set up library definition symbol entry");
E 2
...
I 2
		DBUG_3 ("fnam", "full pathname of modified object is '%s'", fnam);
E 2
...
I 2
		DBUG_3 ("ofil", "modified obj file open on stream %d", ofil);
E 2
...
I 2
			DBUG_2 ("cof3", "write section headers");
E 2
...
I 2
			DBUG_2 ("cof4", "write section contents");
E 2
			for (psect= curobj->sects; psect; psect=psect->next) {
...
I 2
			DBUG_2 ("cof5", "write relocation entries");
E 2
			for (psect= curobj->sects; psect; psect=psect->next) {
...
I 2
		DBUG_2 ("cof6", "write symbol table and regenerate string table");
E 2
		symtaboff= ftell(ofil);

		/* allocate new string table */
I 2
		DBUG_2 ("cof7", "allocate new string table");
E 2
...
I 2
		DBUG_2 ("cof8", "print out the symbol table");
E 2
...
I 2
		DBUG_2 ("cof9", "print the reference chain linking symbols");
E 2
...
I 2
		DBUG_2 ("cof10", "print the file definition symbol");
E 2
...
I 2
		DBUG_2 ("cof11", "print library definition symbol");
E 2
		writesym(&lsymbol, &stringtab, ofil);

        	/* copy string table */
I 2
		DBUG_2 ("cof12", "copy string table");
E 2
...
I 2
		DBUG_2 ("cof13", "write fileheader with correct symbol table info");
E 2
...
I 2
		DBUG_3 ("ofil", "close output file on %d", ofil);
E 2
		(void)fclose(ofil);

		/* add the .init section if one exists */
I 2
		DBUG_2 ("cof14", "add the .init section if one exists");
E 2
		if (curobj->init != NULL) {
...
I 2
	DBUG_ENTER ("archive");
	DBUG_2 ("ar1", "create hstname by archiving all modified objects");
E 2
	/* set cmd */
	...
	(void)sprintf(p,"%s | tsort`", defname);
