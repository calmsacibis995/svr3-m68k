/*	@(#)dis_utls.c	6.2		*/
/*
 *	static	char	dis_utls_ID[] = "@(#)dis_utls.c	6.2 	85/05/31";
 */
#define	DEBUG
#include	<stdio.h>
#include	<ar.h>
...

{
...
#ifdef MC68
	extern char	*cvt_hex_lead,*cvt_hex_nolead;
	extern char	*cvt_oct_lead,*cvt_oct_nolead;
#endif

#ifdef MC68
	if( flag == NOLEAD )
		(oflag) ?	sprintf(temp,cvt_oct_nolead,num):
				sprintf(temp,cvt_hex_nolead,num);
#else
#if UNIX < 4
#ifdef B16
	if (flag == NOLEAD) 
	...
	if (flag == LEAD)
#ifdef MC68
		(oflag) ?	sprintf(temp,cvt_oct_lead,num):
				sprintf(temp,cvt_hex_lead,num);
#else
		(oflag) ?	sprintf(temp,"@%o",num):
				sprintf(temp,"$%x",num);
#endif /* MC68 */
}
/*
...
line_nums()
{
	...
	if (scnhdr.s_lnnoptr != NULL)
	{
		...
		{
			if (fflag)
				ldlinit( l_ptr, currfunc->fcnindex );
			else
				FSEEK( l_ptr, scnhdr.s_lnnoptr, 0 );
			lnno_cnt = 0;
			if (FREAD( (char *) lnno_ptr, LINESZ,1, l_ptr ) != 1)
...
#endif
#ifdef MC68
	extern	char	*print_octal_fmt,*print_hex_fmt;
#endif
...
#ifdef MC68
	(oflag > 0) ?
		printf(print_octal_fmt,object,mneu):	/* to print octal */
		printf(print_hex_fmt,object,mneu);	/* to print hex */
#endif
#ifdef N3B
#ifdef OLIST
...
#ifdef OLIST
	if (oflag)
		sprintf(object,"@%.7lo  ", loc);
	else
		sprintf(object, "$%.6lx  ", loc);
#else
	if (oflag)
