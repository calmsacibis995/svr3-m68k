...
/*		All Rights Reserved		*/
/*	@(#)optim.c	7.3		*/
static	char	SCCSID[] = "@(#) @(#)optim.c	7.3	";
...
simpstr( d, s ) 
{
	...
	sz = tsize( STRTY, d, s );
#ifdef M68020
	/* Special structure alignment for M68020 - see comments in pftn.c */
	al = salign( STRTY, s );
	if( sz == SZINT && !( al % SAINT) ) return( INT );
	else if( sz == SZCHAR && !( al % SACHAR) ) return( CHAR );
	else if( sz == SZLONG && !( al % SALONG) ) return( LONG );
	else if( sz == SZSHORT && !( al % SASHORT) ) return( SHORT );
#else
	al = talign( STRTY, s );
