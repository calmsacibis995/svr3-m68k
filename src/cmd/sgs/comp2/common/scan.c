...
/*		All Rights Reserved		*/
/*	@(#)scan.c	7.5		*/
static	char	SCCSID[] = "@(#) scan.c:	@(#)scan.c	7.5	";
...
#	include <string.h>
#	define TRUE	1
	int stkckflg;	/* global - referenced again in local.c - bfcode() */
#endif
...
int align_struc = 16;	/* M68020 default variable alignment inside
			** structures; word */
#endif
...
#ifdef STKCK			/* flag indicates if stack checking is req'd */
...
ppcontrol( c )
	int	c;
/* For processing if #ident <string> source file line.
/* String passed into ident assember directive line in output code
*/
{
	char	cstring[7];
	int	i;
