/*	@(#)sty.y	7.2		*/
%{
static	char	SCCSID[] = "@(#) sty.y:		@(#)sty.y	7.2	";
/*
...
%token <ival> OP NM DIG STYPE RREG USRNEED NEEDALL
%left OP
%left STYPE
...
nlist	:	/* EMPTY */
	...
	|	nlist RCOUNT
		{	rcount += 1; }
	|	nlist USRNEED
		{	needs |= yylval.ival << NEEDNUM;}
	|	nlist NEEDALL
		{	needs = (needs&~NCOUNT) | ALLREGS | NRGS;}
	;
...
/* tbl_a and tbl_b are file structures for 'tablea.c' and 'tableb.c'. */
/* These two files replace the file table.c because it became too big */
/* to compile when the M68881 information was added to 'stin' */
...
finished()
{
	...
	if( (tbl_a = fopen( "tablea.c", "w" ) ) == NULL )
	{	fprintf( stderr, "sty: unable to open tablea.c\n" );
		exit( -1 );
	}

	if( (tbl_b = fopen( "tableb.c", "w" ) ) == NULL )
	{	fprintf( stderr, "sty: unable to open tableb.c\n" );
		exit( -1 );
	}

	fprintf( tbl_a, "# include \"mfile2.h\"\n\n" );
	fprintf( tbl_b, "# include \"mfile2.h\"\n\n" );
	...
	for(i=0; i<opno; i++)
	{
		...
		if(q->struse>1)
		{
			fprintf( tbl_b, "static char Str%d[] = \"%s\";\n",
				i, q->string);
			q->struse = 0;
			q->strdef = i;
		}
	...
	fprintf( tbl_a, "\n# define SHNL ((SHAPE *)0)\n" );
	fprintf( tbl_a, "# define S(x) (&shapes[x])\n" );
	...
	for( i=0, p=shp; i<nshp; ++i,++p )
	{
		...
		fprintf( tbl_a, "/*%4d */ ", i);
		fprintf( tbl_a, "%4d,\t",p->sop );
		saaddr(p->sleft);
		saaddr(p->sright);
		fprintf( tbl_a, "%#o,\t%d,", p->ssh, p->scost );
		if( p->shname[0] )
			fprintf( tbl_a, "\t/* %.8s */\n", p->shname );
		else
			fprintf( tbl_a, "\n" );
	...
	fprintf( tbl_a, "# define P(x) (&pshape[x])\n" );
	...
	for( j = 0, i = NSTYSHPS-1;  i > nmshp; --i ) {
		...
		fprintf( tbl_a, "/*%4d %.8s */\t", j, shp[i].shname  );
		j = manylist( i, j );
		fprintf( tbl_a, "SHNL,\n");
		++j;
	}
	...
	fprintf( tbl_b, "# define P(x) (&pshape[x])\n" );

	fprintf( tbl_b, "struct optab table[] = {\n\n" );
	for( i=0; i<opno; ++i )
	{
		q = &optb[i];
		fprintf( tbl_b, "/* # %d, line %d */\n", i, q->line );
		prop( q->op );
		prnam( q->tyop, Tnam );
		if( q->nxop >= 0 )
			fprintf( tbl_b, "\t&table[%d],\n", q->nxop );
		else
			fprintf( tbl_b, "\t0,\n" );
		...
		if(q->strdef>=0)
			...
			fprintf(tbl_b, "\t\t\"%s\",\n", q->string);
		fprintf(tbl_b, "\t\t%d,%d,%d,%d,\n",
			q->ocost, q->olcount, q->orcount, q->line );
	}
	fprintf( tbl_b, "\n};\n" );

	fprintf( tbl_b, "OPTAB *ophead[] = {\n" );

	for( i=0; i<DSIZE; ++i )
	{
		if( ophd[i] < 0 ) fprintf( tbl_b, "	0,\n" );
		else fprintf( tbl_b, "	&table[%d],\n", ophd[i] );
	}
	fprintf( tbl_b, "};\n" );
}
...
register sp;
{
	if( sp < 0 ) fprintf( tbl_a, "SHNL,\t" );
	else fprintf( tbl_a, "S(%d),\t", sp );
}
...
register sha,ty;
{
	if( sha < 0 ) fprintf(tbl_b,  "\tPSHNL,\t");
	else fprintf( tbl_b, "\tP(%d),\t", shp[sha].ssh );
	prnam( ty, Tnam );
}
