...
/*		All Rights Reserved		*/
/*	@(#)match.c	7.4		*/
static	char	SCCSID[] = "@(#) match.c:	@(#)match.c	7.4	";
...
# endif
	for( ; q ; q = q->nextop ){
		...
		if( ( q->needs & USERNEEDS ) &&
		    ( ! user_need( p, q ) ) )
			/* continue if user need not satisfied */
			continue;
...
#ifdef M68881
			if( c || 
			  ((q1->tn.type&TPOINT) && !(q2->tn.type&TPOINT)) ||
			  (!(q1->tn.type&TPOINT) && (q2->tn.type&TPOINT)) ||
			  ( is_m68881 &&
			    (FP_TYPE(q1->tn.type) && !FP_TYPE(q2->tn.type)) ||
			    (!FP_TYPE(q1->tn.type) && FP_TYPE(q2->tn.type))))
#else
			if( c || 
			  ((q1->tn.type&TPOINT) && !(q2->tn.type&TPOINT)) ||
...
# endif
	...
	case SRANGE0:
	case SSRANGE:
		if( i<0 || i>30 ) cerror( "bad shape range" );
	case SVAL:
	case SNVAL:
	case NACON:
...
register OPTAB *q;
{
	...
	need = q->needs >> NEEDNUM;
	for( i = 1; need != 0; need >> 1, i << 1 )
	{
		switch( i )
		{
#ifdef M68881
		case FP881:
			...
			return( 0 );
		case R_TEMP:
			/* if register temp return true else return false */
			l = p->in.left;
			r = p->in.right;

			if( l && ( l->in.op == TEMP && l->in.right ) )
				return( 1 );

			if( r && ( r->in.op == TEMP && r->in.right ) )
				return( 1 );

			return(0);
#ifdef FORT
		case INTR_881:
			if( is_m68881 && FP_TYPE( p->in.type ) )
				return( 1 );
			return( 0 );
#endif
#endif
		default:
			cerror( "unknown user need, %x\n", i );
		}
	}
}
