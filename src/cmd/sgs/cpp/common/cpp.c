...
** buffers is particularly noticeable on an Amdahl, where BUFSIZ is 4096.
*/
#    ifdef m68k
#	define SBSIZE (60000 + (MAXFRE+1)*BUFSIZ)
#    else
...
#if m68k && (PD_SYS == D_unix)
STATIC	struct symtab *ysysloc2;	/* kludge to add 'sysV68' def */
#endif
...
/* This is a kludge to get 'sysV68' defined in addition to 'unix', without
** adding a PD_SYS2 notation to cpp.c and the makefiles.  The only real
** clean way to define this is from the makefile, but time doesn't
** permit mucking with it right now. 
*/

#if m68k && (PD_SYS == D_unix)
	ysysloc2 = stsym("sysV68");
#endif
