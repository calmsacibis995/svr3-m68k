...
static char SCCSID[] = " File @(#)scan.c	7.3 Version,  Date of last Edit 9/7/86";
...
#ifdef M68020
/* sdisp -- scan a displacement in a new M68020 addressing mode 
**
...
if (parfound)
    if (*scanp++ != ')' || *scanp != '.')
	serr("Bad sized displacement in M68020 mode");
...
if (*scanp == '.')
	...
	case 'b' : return(0);
	case 'w' : return(2);
	default  : serr("Bad size code in M68020 mode displacement");
	}
    }
...
/* scan_020_mode -- scan across a new M68020 addressing mode
**
...
#ifdef M68020 
	/* detect new M68020 addressing modes */
	tempp = scanp;
...
scanp++;
    ...
    /* check for an appended {o:w} used by M68020 BFxxx instrucs */
    if (*scanp == '{')
    {
	if (*(scanp++) == '%')    /* up to {o    */
