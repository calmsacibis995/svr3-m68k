...
/*	@(#)mipsup.c	7.2		*/
/* mipsup.c */
static char SCCSID[] = "@(#) mipsup.c:	2.1 83/07/08";
...
{
    char labelbuf[10];
    int rlen = sprintf(labelbuf, "L%%%%%d", labelnumber++);
					/* label has form L%%n */
...
NODE * node;				/* instruction to do calc. on */
{
	...
	case MOVM_L:			/* move multiple */
	    retreg |= doreglist(rands[0],rands[1]);
...
NODE * node;				/* instruction to do calc. on */
{
	...
	case MOVM_L:
	    retreg |= doreglist(rands[1],rands[0]);
...
register NODE * node;			/* instruction whose size we calc. */
{
    ...
    */
    ...
    if (isbr(node))
	return(4);			/* size of br (not br.b) */
    
    size = 2;				/* size for op itself */
