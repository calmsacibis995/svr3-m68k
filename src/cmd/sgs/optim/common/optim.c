...
/*		All Rights Reserved		*/
/*	@(#)optim.c	7.3		*/
/*	static char  ID[] = "@(#) optim.c: 1.14 5/25/83";	*/
...
reach(b) register BLOCK *b; { /* recursively mark reachable blocks */
	 ...
	 */
	    ...
	    (isuncbr(bb->lastn) || bb->nextr != NULL || same(bb->lastn, b->lastn))) {
		b->nextl = bb->nextl;
		if (bb == bb->nextl) break;	/* infinite loop fix */
		}
