...
mlinit(){
	...
	lastlink = &(mltree[0].mlhead.nexthead);
	n = &mltree[0];
	for( ; (n++)->mlhead.tag != 0;
...
# endif
		...
		for( ;; ++n ){ /* for each node in multi-level template */
			/* opmatch */
			if( n->mlnode.op < OPSIMP ){
				if( st->in.op != n->mlnode.op )break;
				}
			else {
				register opmtemp;
				if((opmtemp=mamask[n->mlnode.op-OPSIMP])&SPFLG){
					if(st->in.op!=NAME && st->in.op!=ICON && st->in.op!=OREG && 
						! shltype(st->in.op,st)) break;
					}
				else if((dope[st->in.op]&(opmtemp|ASGFLG))!=opmtemp) break;
				}
			/* check shape and type */

			if( ! tshape( st, n->mlnode.nshape ) ) break;
			if( ! ttype( st->in.type, n->mlnode.ntype ) ) break;
