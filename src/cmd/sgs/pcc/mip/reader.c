...
# endif
	...
	case FORCE:
		...
		order( p->in.left, cook = INTAREG|INTBREG );
	...
	case GT:
		...
		if( p->in.right->in.op == ICON && p->in.right->tn.lval == 0 && p->in.right->in.name[0] == '\0' ){
			...
			case GT:
				...
				/* UGE and ULT: Don't set the condition codes */
				/* do side effects and branch or fall through */
			case UGE:
				codgen( tcopy(p->in.left), FOREFF);
