...
/* NO LONGER SUPPORTED - no good reason for existence
   ...
   NO LONGER SUPPORTED */
		...
		case 'f':	/* supply filesystem and inode size (blocks) */
			fflg++;
			super1.s_fsize = atol(argv[i]+2);
			j = 2;
			while(argv[i][j] != '\0' && argv[i][j++] != ':')
				;
			super1.s_isize = atoi(&argv[i][j]) + 2;
			break;
	...
	if(super0.s_magic == FsMAGIC && super0.s_type == Fs16b) {
		if(execvp("/etc/dcopy16b",argv) == -1)
			err("Cannot exec /etc/dcopy16b");
	}
#endif
#if FsTYPE==1
	if(super0.s_magic == FsMAGIC && super0.s_type == Fs2b) {
...
#endif
#if FsTYPE==5
	if(super0.s_magic == FsMAGIC && super0.s_type == Fs1b) {
		if(execvp("/etc/dcopy1b",argv) == -1)
			err("Cannot exec /etc/dcopy1b");
	}
	if(super0.s_magic == FsMAGIC && super0.s_type == Fs2b) {
...
#if FsTYPE==2
	if(super0.s_magic != FsMAGIC ||
	(super0.s_magic == FsMAGIC && (super0.s_type == Fs1b || super0.s_type == Fs16b))) {
		err("%s not a 1k file system",argv[0]);
	}
#endif
#if FsTYPE==5
	if(super0.s_magic != FsMAGIC ||
	(super0.s_magic == FsMAGIC && (super0.s_type == Fs1b || super0.s_type == Fs2b))) {
		err("%s not an 8k file system",argv[0]);
	}
#endif
	/*
...
/* for both type 2 and type 5, get the superblock in the same place */
#if (FsTYPE==2 || FsTYPE==5)
getsblk(p, bno)
char *p;
daddr_t	bno;
