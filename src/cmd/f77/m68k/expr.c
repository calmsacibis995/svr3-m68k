...
switch(opcode)
	...
	case OPSTAREQ:
		...
		if( ISCOMPLEX(ltype) || ISCOMPLEX(rtype) )
			break;
#ifdef SGS2
		if( ONEOF(ltype, MSKINT) && ONEOF(rtype, MSKINT)
#else
...
#if TARGET==M68K
			p->datap->addrblock.memoffset = (expptr) NULL;
#endif
		}
	else if(qtag==TPRIM && q->primblock.argsp==0 &&
...
/* If real**int, type of function is really double (it is a C function).
 * Thus, treat as TYDREAL and put OPCONV(TYREAL) over node.
 */

		q = call2((ltype == TYREAL) ? TYDREAL : ltype,
			powint[ltype-TYLONG], lp, mkconv(TYLONG, rp));
		if (ltype == TYREAL)
			q = mkconv(TYREAL, q);
		}
	}
	...
	{
	q = call2(TYDREAL, "pow_dd", mkconv(TYDREAL,lp), mkconv(TYDREAL,rp));
	if (mtype == TYREAL)
		q = mkconv(TYREAL, q);
	}
else	{
