...
#if m68k
static char *hloptname  = HLOPTNAME ;
#endif
...
/* argument passing error, every character in name becomes a flag 
			break;
   change break to the following*/
			goto endfor;
...
#endif
#ifdef TARGET == M68K
		break;		/* if flag is not ignored it is passed to */
				/* ld which causes problems  */
#endif
...
#if HERE==PDP11 || HERE==VAX || HERE==M68K
			fprintf(diagfile, "%s:\n", argv[i]);
#endif
...
#if TARGET != PDP11
if( (fp = fopen(asmfname, "a")) == NULL)	/* this is to get the */
...
if( hlflag )
{
	sprintf(buff, "%s %s %s", hloptname, textfname, hoptfname);
	if( sys(buff) )
		goto error;
	strcpy( hoptfname, textfname );
}
#endif
...
#if TARGET==PDP11 || TARGET==VAX || TARGET==X3B || TARGET==M68K
#	ifdef PASS2OPT
	if(optimflag)
		{
...
#if TARGET == VAX || TARGET == X3B || TARGET==M68K
	/* vax and 3B assemblers currently accept only one input file */
...
#if TARGET!=INTERDATA && TARGET!=PDP11 && TARGET!=VAX && TARGET!=X3B && TARGET!=M68K
...
#if HERE==PDP11 || HERE==INTERDATA || HERE==VAX || HERE==X3B || HERE==M68K
	if( (waitpid = fork()) == 0)
		{
		enbint(SIG_DFL);
		execv(ldname, v0);
