...
for (p = p0 ; p ; p = p->nextp)
	...
	if(q->tag == TIMPLDO)
		{
#if TARGET==M68K
		exdo(range=getlab(), q->impldoblock.impdospec);
#else
...
if(statstruct && ISCONST(p))
	{
	setdata(q, p, typesize[type], blklen, 1L);
	frexpr(q);
	frexpr(p);
	}
else
...
#if TARGET==M68K
   {    cp = ICON(0);
	cp->constblock.vstg = STGCONST;
	ioset(TYLONG, offset, cp );
   }
#else
