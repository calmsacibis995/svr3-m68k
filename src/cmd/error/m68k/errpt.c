static char sccsid[] = "@(#)errpt.c	6.4		";
/*
...
#ifdef m68k
#define major(x)	(int)((unsigned)(x>>8)&037)
#else
...
#ifdef m68k
#define PACKET_LENGTH	ercd.ebb.reginf[1] >> 8
#define U_DEV_NUM    	ercd.ebb.reginf[1] &  077
#define CPUBASE	680
#endif
...
#ifdef m68k
char *dev[] = {
	"UD0", "WD0","M320","M350","M355","xx5","M360",
	0
};
#else
char *dev[] = {
...
struct regs udregs9 [] = {     /* 9-byte status packet */
	"$02:PID", NULS,
	"LNG:DNO", NULS,
	"CMD:STY", NULS,
	"STATUS ", NULS,
...
struct regs udregs13 [] = {     /* 13-byte status packet */
	"$02:PID", NULS,
	"LNG:DNO", NULS,
	"CMD:STY", NULS,
	"STATUS ", NULS,
...
struct regs udregs15 [] = {     /* 15-byte status packet */
	"$02:PID", NULS,
	"LNG:DNO", NULS,
	"CMD:STY", NULS,
	"STATUS ", NULS,
...
struct regs udregs16 [] = {     /* 16-byte status packet */
	"$02:PID", NULS,
	"LNG:DNO", NULS,
	"CMD:STY", NULS,
	"STATUS ", NULS,
...
struct regs udregs19 [] = {     /* 19-byte status packet */
	"$02:PID", NULS,
	"LNG:DNO", NULS,
	"CMD:STY", NULS,
	"STATUS ", NULS,
...
struct regs m350regs [] = {
	...
	"ABTVEC", NULS,
	"TAS",	NULS,
    /* Need 8 fillers, cause 350 driver currently reports it has saved
    ** twice as many regs as it really has...  -wjc
    */
	"Z", NULS,
	"Z", NULS,
	"Z", NULS,
...
*	ADD THIS STUFF TO struct m350regs SOMEDAY	*
...
struct regs m360regs [] = {
	/* These pseudo registers are actually a collection of important
	 * data items placed in struct m360elog in the driver at the time
	 * of the error.*/
	"BOFF-HI",NULS,		/* block offset of error */
	"BOFF-LO",NULS,
	"DRVSTAT",NULS,		/* status of two drives */
	"CSR    ",NULS,		/* command & status reg */
	"CMD:OPT",NULS,		/* command, cmd options */
	"STA:ERR",NULS,		/* status, error */
	0
};
...
struct pos {
	...
	unsigned sector;
#ifdef m68k
	int blkoff;	/* block offset of error */
#endif
} pos;
...
#endif
#ifdef m68k
/*25*/	"\tBlock Offset of I/O\t\t%16u\n",
/*26*/	"\tMinor Device Number\t\t0x%X\n",
#endif
	0
};
...
/* 4*/	"\tAt Location\t",
#ifdef m68k
/* 5*/	"\n\nMEMORY PARITY ERROR: Single-Bit Error (Corrected) at %s\n",
/* 6*/	"\n\nMEMORY PARITY ERROR: Uncorrectable Error at %s\n",
/* 7*/	"\tMemory Board CSR Address\t%.8X\n",
/* 8*/	"\tMemory Board Syndrome Bits\t%.2X\n",
#else
#ifdef pdp11
...
/*13*/	"\n\n\tSystem Profile:\n\n",
#ifdef m68k
/*14*/	"\t     Motorola  MC680%d0  Processor\n",
#else
/*14*/	"\t     11/%d  Processor\n",
#endif
...
/*16*/	Nl,
#ifdef m68k
/*17*/	"\t     SYSTEM V/68 Release %s Operating System (%s)\n",
#else
...
#ifdef m68k
		printf(xlines[14], est.e_cpu - CPUBASE);
#else
...
party()
{
	...
	if (print == YES) {
		...
		if (page > limit)
			return;
#ifdef m68k
		printf(xlines[(erm.e_memtyp == E_SBME) ? 5 : 6],
			ctime(&e_hdr.e_time));
		printf(xlines[7], erm.e_memcsr);
		printf(xlines[8], erm.e_memsyn);
#else
...
blk()
{
	...
	for(i = 0; i < r->e_nreg; i++, regs_ptr++) {
		if (*regs_ptr->regname != 'Z') {
			printf("\t  %-7.7s\t   ", regs_ptr->regname);
			printf("%4.4x", ercd.ebb.reginf[i]);
...
udblk()
{
	...
	case 19:
		...
		break;
	default:
		fprintf (stderr, "Invalid block device number.\n");
...
m360blk()
{
	...
	pos.flg = 1;
	pos.unit = min;	/* preserve real minor dev number */

	/* squeeze unused bits from minor number */
	ctlr = min & 0x80;
	min =  min & 037 | ctlr >> 2; 

	/* get block offset of error from first two pseudo regs */
	pos.blkoff = (ercd.ebb.reginf[0] << 16) | ercd.ebb.reginf[1];
}
