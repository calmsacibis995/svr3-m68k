...
main()
{
	 ...
	 */
		...
		FPRINTF(Debugfd, "(%5d) open NS_PIPE ok\n", Mypid);
		FFLUSH(Debugfd);
		(void) poptli(fd);
		...
		FPRINTF(Debugfd, "(%5d) tirdwr pushed on stream\n", Mypid);
		FFLUSH(Debugfd);
		...
		FPRINTF(Debugfd, "(%5d) fd in NS_PIPE", Mypid);
		FFLUSH(Debugfd);
		(void) exit(0);
	case RF_RF:
...
int fd;
{
	...
	FPRINTF(Debugfd, "(%5d) enter banter\n", Mypid);
	FFLUSH(Debugfd);
	...
	FPRINTF(Debugfd, "(%5d) received something\n", Mypid);
	FFLUSH(Debugfd);
	...
	FPRINTF(Debugfd, "(%5d) returning from banter\n", Mypid);
	FFLUSH(Debugfd);
	return(rpnhdr.pn_hi);
}
