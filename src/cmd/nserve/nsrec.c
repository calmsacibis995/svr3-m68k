...
r_init()
{
	if (getprime(dompart(Dname)) == FAILURE)
		return(FAILURE);
	...
	if (getnsinfo() == FAILURE)
		return(FAILURE);

	if (setrstate() == FAILURE)
		return(FAILURE);
...
getmyaddr()
{
	...
	sprintf(buf,"%s -z %d %s >/dev/null 2>&1",NLS_Q,RFS,Net_spec);
...
setrstate()
{
	...
	for (i=0, np=Nsinfo; i < MAXNS; i++, np++) {
		...
		if (np->n_pd == -1) {
			...
			if (r_sndreq(np->n_pd, np, QUERY, NS_IM_NP) == FAILURE)
				np->n_state = R_DEAD;
			else
				np->n_state = R_UNK;
		}
	...
	while (Rdone == FALSE) {
		...
		case REC_ACC:
			...
			if (r_sndreq(pd, np, QUERY, NS_IM_NP) == FAILURE) {
				nsrclose(pd);
				np->n_state = R_DEAD;
				np->n_pd = -1;
			}
		...
		case REC_HUP:
			...
			nsrclose(pd);
			if (np) {
				/* modification for fast connection transport
				   providers.  will retry NRETRY and then
				   set to dead to allow alarm to fire in a
				   reasonable amount of time - dtk */
				if (np->n_retry++ < NRETRY) {
					if ((np->n_pd = rconnect(np->n_addr,RECOVER)) == -1)
						np->n_state = R_DEAD;
					else
						np->n_state = R_PENDING;
...
int	type;
{
	...
	case REC_ACC: /* got reply to checkup connection, send poll.	*/
		if (r_sndreq(pd, np, QUERY, (Primary)?NS_IM_P:NS_IM_NP) == FAILURE) {
			nsrclose(pd);
			np->n_state = R_DEAD;
			np->n_pd = -1;
		}
...
int		pd;
{
	...
	case NS_IM_P:
		if (Primary) {
			...
			if (r_sndreq(pd, np, QUERY, NS_IM_P) == FAILURE) {
				nsrclose(pd);
				np->n_state = R_DEAD;
				np->n_pd = -1;
		    		return(setrstate());
...
nsrel()
{
	...
	for (i=0, np = Nsinfo; i < MAXNS; i++, np++)
		if (np->n_state == R_NOTPRIME && strcmp(Dname,np->n_dname)) {
			if (r_sndreq(np->n_pd, np, QUERY, NS_REL) == FAILURE) {
				nsrclose(np->n_pd);
				np->n_pd = -1;
				np->n_state = R_DEAD;
			}
