...
char	*argv[];
{
	...
	static int	here_before = 0;	/* MC debug */

	if ( here_before ) {
		LOG1(L_OVER,"nserve: entering main\n");
		fflush(Logfd);
		abort();
	}
	else {
	...
	if (!Oedipus && fork()) {
		LOG1(L_OVER,"nserve: Oedipus not set - forking\n");
		exit(0);
	}
	LOG1(L_OVER,"nserve: after Oedipus fork\n");
...
struct request	*oreq;	/* request, if non-NULL, contains undo info	*/
{
	...
	if (hp == NULL) {
		...
		/* test oreq to see if it was passed as a NULL flag */
