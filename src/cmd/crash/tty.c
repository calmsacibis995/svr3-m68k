...
/* List of currently known drivers supporting a tty interface */
...
static char Lasttype[20] = {'m', '3', '3', '1', 0};
static char *Lastptr = &Lasttype[0];
...
int flg;
{
	...
	if(!(sp = symsrch(temp)))  {
	    if (flg)   return (0);
	    else       error("%s not found in symbol table\n",type);
	}
	return(sp->n_value);
}
...
gettty()
{
	...
	if((!strcmp("",type)) && !args[optind]) {
		for (typedex = 0; ttyp = types[typedex]; typedex++)  {
		    if ((addr = getaddr(ttyp, 1)) == 0)
			continue;
		    count = getcount(ttyp);
...
*/
		...
		if(strcmp(type,"")) {
			...
			addr = getaddr(Lastptr, 0);
			count = getcount(Lastptr);
			fprintf(fp,"\n%s TABLE SIZE = %d\n",Lasttype, count);
		}
