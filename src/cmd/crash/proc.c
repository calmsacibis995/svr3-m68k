...
char *heading;
{
	...
	fprintf(fp,"%4d %c %5u %5u %5u %5u  %2u %3u",
		...
		procbuf.p_pri,
		procbuf.p_cpu);
	fprintf(fp," %08lx ",procbuf.p_wchan);
	if(procbuf.p_stat == SZOMB)
		cp = "zombie";
	else {
		/* get ublock */
		pgaddr = ctob(pg_getpfn(procbuf.p_ubpgtbl));
		if (lseek(mem, pgaddr,0) == -1) {
				prerrmes("seek error on ublock address\n");
				return;
		}
		if((read(mem,(char *)ubp,sizeof(*ubp))) != 
		sizeof(*ubp))
		{
	...
	for(i = 0,j = 1;  i < USIZE + 7  ;  i++) 
		if(*(int *)&procbuf.p_ubpgtbl[i] != 0) {
			if(!(j++ & 3))
				fprintf(fp,"\n\t");
			fprintf(fp,"%d: %8x   ",i,procbuf.p_ubpgtbl[i]);
		}	
	...
	printf("pregpp= %d    start of p_reg table %x\n", pregpp, procbuf.p_region);
	for( i = 0; i < pregpp; i++) {
	readmem((long)(procbuf.p_region+i),1,slot,
		...
		if(!pregbuf.p_reg) 
			break;
		fprintf(fp,"\t%4d ",i);
	printf("addr of cell (%x)   start of Region table (%x)\n",
	pregbuf.p_reg, Region->n_value);
