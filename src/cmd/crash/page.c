...
unsigned size;
{
	...
	offset = kpbase * sizeof pfbuf;
	readbuf(addr,(long)(pfdat+ offset +slot*sizeof pfbuf),phys,-1,
		(char *)&pfbuf,sizeof pfbuf,"pfdata table");
	if(addr > -1)
		slot = getslot(addr,(long)&pfbuf,sizeof pfbuf,phys);
	if(!pfbuf.pf_flags && !all)
		return;
	/* calculate pfdata entry number of pointers */
	next = ((long)pfbuf.pf_next - (long)&pfbuf)/sizeof pfbuf;
	if ((next < 0) || (next > size)) next = -1;
	prev = ((long)pfbuf.pf_prev - (long)&pfbuf)/sizeof pfbuf;
	if ((prev < 0) || (prev > size)) prev = -1;
	hash = ((long)pfbuf.pf_hchain - (long)&pfbuf)/sizeof pfbuf;
	if ((hash < 0) || (hash > size)) hash = -1;
		sizeof (struct inode);
	if(addr > -1) 
		slot = getslot(addr,(long)&pfbuf,sizeof pfbuf,phys);
	fprintf(fp,"%4d %8d %3d %3u,%-3u %5d  %3d  %3d ",
		slot,
		pfbuf.pf_blkno,
		pfbuf.pf_use,
		major(pfbuf.pf_dev),
...
getregion()
{
	...
	long arg2 = -1;
	int c;
	char * heading = "SLOT LSZ PSZ POFF #VL REF    FSIZE TSTM TYPE FOR  BCK INO  LISTADDR FLAGS\n";
...
char *heading;
{
	...
	if(full)
		fprintf(fp,"%s",heading);
	fprintf(fp,"%4d %3d %3d %4d %3d %3d %8x %4d ",
		slot,
		rbuf.r_listsz,
		rbuf.r_pgsz,
		rbuf.r_pgoff,
	...
	fprintf(fp,"%s%s%s%s%s%s%s",
			...
			rbuf.r_flags & RG_REF ? " ref" : "",
			rbuf.r_flags & RG_SSWAPMEM ? " swap" : "",
			rbuf.r_flags & RG_NDREF ? " ndref" : "");
	if(full) { 
		if(rbuf.r_pgsz) 
