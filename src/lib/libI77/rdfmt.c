...
{	int ch;
	...
	*ps='\0';
	ps=s; while (*ps==' ') ps++;
	if (*ps=='.') ps++;
	if (*ps=='t') { *n=1; return(0); }
	else if (*ps='f') { *n=0; return(0); }
	else return(errno=116);
}
extern double ten_pow[] /* = {1e1,1e2,1e3,1e4,1e5,1e6,1e7,1e8,1e9} */ ;
...
*sp='\0'; sp=s;
while (*sp==' ') sp++;
if (*sp=='-') { sx=1; sp++; } else { sx=0; if (*sp=='+') sp++; }
loop1:
while (*sp>='0' && *sp<='9') {
		x=x*10+(*sp-'0');
		sp++;
		}
if (*sp==' ') { if (cblank) x*=10; sp++; goto loop1;}
nfrac=0;
if (*sp=='.') {
	sp++; dot=0;
	loop2:
	...
	if (*sp==' ') {if(cblank) {x*=10; nfrac--;} sp++; goto loop2;}
	}
if (*sp=='d' || *sp=='e') { sp++; } else nfrac-=scale;
while (*sp==' ') sp++;
if (*sp=='-') { sp++; se=1; } else { se=0; if (*sp=='+') sp++; }
exp=0;
loop3:
while (*sp>='0' && *sp<='9') {
	exp=exp*10+(*sp-'0');
...
if (*sp==' ') {if(cblank) exp*=10; sp++; goto loop3;}
if (se) exp=nfrac-exp; else exp += nfrac;
if (dot) exp -= d;
if (exp>0) {
	while (exp>9) { x *= 1e9; exp -= 9; }
	x *= ten_pow[exp-1];
	}
else if (exp<0) {
	exp = -exp;
	while (exp>9) { x *= 1e-9; exp -= 9; }
	x /= ten_pow[exp-1];
	}
if (sx)  x= -x;
if (len==sizeof(float)) p->pf=x; else p->pd=x;
if (*sp) return(errno=115); else return(0);
}
