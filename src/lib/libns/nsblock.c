...
/*	Pointer to null terminated string - to avoid improper
	use of location zero.  Location zero is invalid with
	MCS 413 object file format. Danger!!!
*/
static char	nuthin[] = "";
static char	*NULL_PTR = nuthin;
...
struct res_rec	*rp;
{
	...
	case RN:
		...
		if ((rp->rr_desc = getstr(pp,NULL,NULL)) == NULL)
			rp->rr_desc = NULL_PTR;
		if ((rp->rr_path = getstr(pp,NULL,NULL)) == NULL)
			rp->rr_path = NULL_PTR;
