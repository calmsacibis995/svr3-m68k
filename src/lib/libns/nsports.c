...
struct address	*addr;
{
	...
	LOG3(L_COMM,"(%5d) nsconnect: mode = LOCAL, address = %s\n",
		Logstamp, (addr)?aatos(Logbuf,addr,RAW):"NULL");
 ...
 *
 *	added test and t_close call due to T_DISCON_IND remaining
 *	in lookbuf and next user of fd will receive indication
 */
int
nsclose(pd)
int	pd;
{
	...
	pptr->p_mode = UNUSED;
	if (pptr->p_fd != -1) {
		if ( _t_checkfd(pptr->p_fd) == NULL ) {
			/* just a normal file descriptor, not TLI */
			close(pptr->p_fd);
		}
		else {
			/* TLI file descriptor */
